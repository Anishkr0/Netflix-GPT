...
import { NETFLX_LOGO } from "../utils/constant";
...
const Header = () => {
  ...
  return (
    <div className="absolute  px-9 py-2 bg-gradient-to-b from-black z-40 flex justify-between w-full items-center">
      ...
      {user && (
        <div className="flex">
  <button className="px-2 py-2 m-2 text-white bg-black rounded-2xl" onClick={handleGptSearchClick}> GPT Search</button>
          <img className="w-13 h-12  rounded-full" src={user?.photoURL} />
          <button className= " text-white bg-black  px-2 py-2 m-2  rounded-2xl" onClick={handleSignOut}>
            (SignOut)
          </button>
        </div>
      )}
